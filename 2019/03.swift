...
/// This is an awful mess. Save your time and don't read it.
...
func makeDirection(_ string: String) -> Direction {
  let dir = string[string.startIndex]
  let s = String(string[string.index(after: string.startIndex)...])
  let len = Int(s)!
...
var wirePaths: [Point: ((Bool, Int), (Bool, Int))] = [:]

func calcPaths(_ wire: Int, _ directions: [Direction]) {
  ...
  directions.forEach { direction in
    ...
    case .down(let len):
      for _ in 1...len {
        steps += 1
        let newPoint = Point(x: acc.x, y: acc.y - 1)
        if wirePaths[newPoint] == nil {
          wirePaths[newPoint] = wire == 1
            ? ((true, steps), (false, 0)) : ((false, 0), (true, steps))
        } else {
          wirePaths[newPoint] = wire == 1
            ? ((true, steps), wirePaths[newPoint]!.1) : (wirePaths[newPoint]!.0, (true, steps))
        }
        acc = newPoint
      }
    case .up(let len):
    ...
    case .left(let len):
      for _ in 1...len {
        steps += 1
        let newPoint = Point(x: acc.x - 1, y: acc.y)
        if wirePaths[newPoint] == nil {
          wirePaths[newPoint] = wire == 1
            ? ((true, steps), (false, 0)) : ((false, 0), (true, steps))
        } else {
...
func distanceFromCenter(_ point: Point) -> Int {
